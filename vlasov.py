...
class accelerator:
    def __init__(self, circumference, mass, charge, beta, Q_b, Q_s, alpha, chromaticity, num_bunches):
...
class vlasovSolver():
    ...
    def impedanceSampleFrequencies(self, max_harmonics, multi_bunch_mode):
        ...
        fp = (p * self.accelerator.num_bunches + multi_bunch_mode) * self.accelerator.f0 + self.accelerator.w_b / 2 / np.pi
    ...
    def generateInteractionMatrix(self, N):
        ...
        interactionMatrix = lm.generateInteractionMatrix(
            self.base_matrix, self.l_matrix,
            N, self.accelerator.num_bunches, self.accelerator.mass, self.accelerator.beta,
            self.accelerator.f0, self.accelerator.w_b, self.accelerator.w_s)
...
class NHT(vlasovSolver):
    ...
    def calculateRingRadii(self, g0hat, search_limit):
        self.ring_radii = nht.calculateRingRadii(g0hat, self.num_rings, search_limit)
...
class arbitraryLongitudinal(vlasovSolver):
    ...
    def calculateGk(self, g0hat, a, terms, max_radius=1, numerical_normalise=False, *args, **kwargs):
    ...
    def generateBaseMatrix(self, sample_frequencies, sampled_dipole_impedance):
        if len(self.Gk) == 0:
            raise Exception("Unperturbed longitudinal distribution has not been fit. \n " +\
            "Run calculateGk() before calculating a base matrix.")
            return self.base_matrix
